...
class FeatureCatalogService {
   ...
   */
  async updateFeature(featureKey, updateData) {
    try {
   ...
   * @returns {Array} 配置列表
   */
  async getFeatureConfigurations(featureId) {
    try {
   ...
   */
  async setFeatureConfigurations(featureId, configurations) {
    try {
      const trx = await db.transaction();

      try {
        // 删除现有配置
        await trx('feature_configurations')
          .where('feature_id', featureId)
          .del();

        // 插入新配置
        const configData = configurations.map((config, index) => ({
        ...
        // 清除缓存
        await cacheService.del(`${this.cachePrefix}config:${featureId}`);

        logger.info(`[FeatureCatalogService] Set ${insertedConfigurations.length} configurations for feature: ${featureId}`);
   ...
   * @returns {Array} 权限列表
   */
  async getFeaturePermissions(featureId) {
    try {
   ...
   */
  async checkFeatureAccess(featureKey, userId, userContext = {}) {
    try {
      const feature = await this.getFeatureByKey(featureKey);
      if (!feature) {
        return false;
   ...
   * @returns {boolean} 是否有权限
   */
  async evaluatePermission(permission, userId, userContext) {
    try {
      switch (permission.permission_type) {
        ...
        case 'role':
          return userContext.roles && userContext.roles.includes(permission.permission_value);
   ...
   */
  evaluateCustomConditions(conditions, userContext) {
    try {
   ...
   */
  async recordFeatureUsage(featureKey, userId, usageData = {}) {
    try {
      const feature = await this.getFeatureByKey(featureKey);
      if (!feature) {
        return;
          ...
          await trx('feature_usage_stats')
            ...
            .update({
              ...
              usage_metrics: JSON.stringify({
                ...JSON.parse(existingStat.usage_metrics || '{}'),
                ...metrics
              }),
              total_cost: parseFloat(existingStat.total_cost) + parseFloat(cost),
              status: status === 'failed' ? 'failed' : existingStat.status,
              error_details: errorDetails ? JSON.stringify(errorDetails) : existingStat.error_details,
              updated_at: new Date()
            });
        } else {
          // 创建新记录
      ...
      logger.debug(`[FeatureCatalogService] Recorded usage for feature: ${featureKey}, user: ${userId}`);
    ...
    try {
      ...
      switch (groupBy) {
        case 'feature':
          ...
          break;
        case 'user':
          query = query.select('fus.user_id')
            .groupBy('fus.user_id', 'fus.feature_id', 'fd.feature_key', 'fd.name', 'fd.category');
          break;
        case 'day':
        default:
          query = query.groupBy('fus.usage_date', 'fus.feature_id', 'fd.feature_key', 'fd.name', 'fd.category');
          break;
      }
...
module.exports = new FeatureCatalogService();