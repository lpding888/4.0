...
jest.mock('../src/db', () => {
  ...
  (mockDb as any).transaction = jest.fn((callback) => {
    // 模拟事务，直接执行回调
    return callback(jest.fn((table: string) => {
...
describe('配额Saga流程集成测试', () => {
  ...
  beforeAll(async () => {
    ...
    await mockDb('pipeline_schemas').insert({
      ...
      steps: JSON.stringify([
        { type: 'test-step', provider_ref: 'test-provider', timeout: 5000 },
      ]),
    });
  });
  ...
  describe('成功场景 - 完整Saga流程', () => {
    it('应该成功执行完整的Saga流程', async () => {
      ...
      expect(pipelineEngine.executePipeline).toHaveBeenCalledWith(
        taskId,
        featureId,
        inputData
      );
  ...
  describe('失败场景 - 配额不足', () => {
    it('应该在配额不足时抛出错误', async () => {
      ...
      // 执行测试并验证异常
      await expect(taskService.createByFeature(userId, featureId, inputData))
      ...
      // 验证没有配额交易记录
      const transaction = await mockDb('quota_transactions')
