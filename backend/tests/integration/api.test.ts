...
describe('API集成测试', () => {
  ...
  afterAll(async () => {
    // 清理测试数据
    await db('verification_codes').where('phone', 'like', '138001380%').del();
    await db('users').where('phone', 'like', '138001380%').del();
    await db('tasks').where('user_id', 'like', 'test-%').del();
    await db.destroy();
  });
  ...
  describe('认证接口', () => {
    describe('POST /auth/login', () => {
      test('验证码登录应该成功', async () => {
        // 创建验证码
        const database = db;
        await database('verification_codes').insert({
          phone: '13800138002',
          code: '123456',
          created_at: new Date()
        });
